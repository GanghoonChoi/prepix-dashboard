...
import { Button, Switch, Card, CardHeader, CardContent, CardTitle, CardDescription } from "@heroui/react";
import {
  Modal,
  ModalBackdrop,
  ModalContainer,
...
export default function SettingsPage() {
  ...
  const handleNotification = async (key: string, value: boolean) => {
    ...
    try {
      await userService.updateNotifications(next);
    } catch {
      setNotifications(prev);
    }
  };

  const inputClass = "w-full rounded-md border border-border bg-field-background px-3 py-2 text-sm text-foreground outline-none focus:ring-1 focus:ring-foreground/20";
    ...
    <div className="max-w-3xl space-y-8">
      ...
      <Card>
        ...
        <CardContent className="divide-y divide-border">
          {loading ? (
            <div className="py-4"><Skeleton className="h-10 w-full rounded" /></div>
          ) : (
            <>
              <Row
                label="Name"
      ...
      <Card className="border-danger/30">
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            ...
            <Button variant="danger-soft" size="sm" onPress={() => deleteModal.open()}>
              Delete
            </Button>
          </div>
        </CardContent>
      ...
      <Modal state={passwordModal}>
        ...
        <ModalContainer>
          <ModalDialog>
            ...
            <ModalBody>
              <p className="mb-4 text-xs text-muted">You&apos;ll be logged out after changing.</p>
              <div className="space-y-3">
                <div>
        ...
        <ModalContainer>
          <ModalDialog>
            ...
            <ModalFooter>
              ...
              <Button variant="danger" size="sm" onPress={() => { flash("error", "Please contact support to delete your account."); deleteModal.close(); }}>
                Delete account
              </Button>
            </ModalFooter>
          </ModalDialog>
